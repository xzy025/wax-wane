import { Activity, ArrowUpCircle, ArrowDownCircle, BarChart2, RefreshCw, TrendingUp, TrendingDown, Gauge } from 'lucide-react'
...
export default function AShareBanner({ t }: AShareBannerProps) {
  const { data, loading, error, lastUpdated, refresh } = useAShareData()
  const hasData = !!data

  const score = data ? calcProfitabilityScore(data.limitUpCount, data.limitDownCount, data.advance, data.decline) : 0
    ...
    <>
      <div className="ashare-banner">
        ...
        {hasData && (
          <>
            ...
            <div className="ashare-stat">
              ...
              <div>
                <div className="ashare-stat-label">{t.ashare.advance}/{t.ashare.decline}</div>
                <div className="ashare-stat-value">{data.advance}/{data.decline}</div>
              </div>
            </div>
            ...
            <div className={`ashare-score ${getProfitabilityClass(score)}`}>
              ...
              <div>
                ...
                <div className="ashare-score-value">{score}<span className="ashare-score-unit">/100</span></div>
        ...
        <div className="ashare-meta">
          {lastUpdated && (
            <span>{t.ashare.lastUpdated} {lastUpdated.toLocaleTimeString()}</span>
          )}
