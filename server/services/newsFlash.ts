...
//
// 双源互备:东财 getFastNewsList(主源,stockList 即项目 secid 口径)+ 新浪 zhibo
// feed(备源,实时性最强)。两源 allSettled 各自归一化后合并去重(东财优先),
// 单源失联降级为单源;双源全挂 throw 交给 createCache serve-stale。
// 快讯 7x24 全天滚动,盘后不能像行情那样给 12h 长 TTL——盘中 30s / 盘后 5min。
// 国内源直连,不走代理;零 header 零签名(2026-07-07 实测)。
...
async function computeNewsFlash(): Promise<NewsFlashData> {
  const [em, sina] = await Promise.allSettled([fetchJson(EM_URL), fetchJson(SINA_URL)])
  ...
  const sources = { eastmoney: emItems.length > 0, sina: sinaItems.length > 0 }
  if (!sources.eastmoney && !sources.sina) {
    throw new Error('[NewsFlash] 东财+新浪双源全部失败,保留既有缓存')
  }
  return {
    asof: new Date().toISOString(),
    items: mergeFlashItems([emItems, sinaItems]),
    sources,
  }
}
