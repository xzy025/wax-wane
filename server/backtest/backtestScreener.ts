...
import { classify, type Bar } from '../services/screenerRules'
...
function simulate(sb: StockBars, cfg: ScreenerConfig, group: 'breakout' | 'trigger' = 'breakout', hold = HOLD): Trade[] {
  const { bars } = sb
  const len = bars.length
  ...
  while (i <= len - 2) {
    ...
    trades.push(makeTrade(sb.code, bars, i, entry, stop, target, risk, simForward(bars, i, stop, target, hold)))
...
function runConfig(data: StockBars[], cfg: ScreenerConfig): { metrics: Metrics; trades: Trade[] } {
  const trades = data.flatMap((sb) => simulate(sb, cfg))
...
async function main() {
  const t0 = Date.now()
  console.log(`[Backtest] 配置 SAMPLE=${SAMPLE} KLINE=${KLINE} HOLD=${HOLD} 并发=${CONCURRENCY} 扫描=${DO_SWEEP}`)
