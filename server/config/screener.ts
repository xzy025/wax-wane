...
// ════════════════════════════════════════════════════════════════════════
// 大盘反攻日·先锋股(reboundDay)—— 复盘卡 + 回测裁决用;**非战法非买点**。
...
//
// ── 回测裁决(2026-07-10,REBOUND=1,SAMPLE=300/KLINE=700,事件闸门=上证 reversalByDate)──
// 结论:**样本不足,不接战法 tab,复盘卡为最终形态**。回看窗 2023-07~2026-07 全程仅 6 个反攻日事件
// (2023-07-25/09-22、2024-01-24/10-18/11-27、2025-01-14;2025-2026 慢牛几乎无此形态,
// 07-09 原型在样本K线区间外未入评)——长电型 n=5/事件2、东山型 n=32/事件4,均未达双下限
// (n≥30 且 事件≥10)。方向性观察(小样本,不作数):东山型一致为正(尾盘 0.12R/PF1.44、
// 次日开盘 0.16R、HOLD10 0.20R),长电型默认负(-0.11R)、去掉放量闸门(VOL_RATIO_MIN=1.0)
// 反而 1.00R/PF6.76/n17="缩量反攻日更好"存疑待样本;闸门对照:同个股条件 全时段 vs 反攻日
// **无可测增量**(低位首板·任意日·次日开盘 0.24R/PF1.52/n432 vs 反攻日 kept 0.06R/n3——
// 这条"低位首板次日跟进"全时段基线本身值得另立课题)。事件年频≈2,凑足事件≥10 需 5 年+数据,
// 超出数据可得性;判据放宽(UP1.2/连跌2)也只到事件3,无法制造样本。
...
// 股东户数确认因子(移植计划⑤数据侧,纯展示·不进规则层·不影响回测):吸筹监控组挂最新一期
// 户数环比——户数下降=筹码集中,与「持续放量横盘」互为佐证。⚠回测裁决通过前不进任何战法评分
// (季度披露口径有前视风险,FUNDRES 披露日前视坑已付过学费;展示带披露日供人工判断时效)。
