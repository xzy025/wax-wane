...
export async function fetchHotList(): Promise<HotListData> {
  ...
  // Fetch dragon tiger from API (works)
  const dragonTiger = await fetchDragonTiger()

  cachedData = {
    eastmoney: getMockEastMoney(),
    ths: getMockTHS(),
    taoguba: getMockTaoguba(),
    dragonTiger: dragonTiger.length > 0 ? dragonTiger : getMockDragonTiger(),
  }
  cacheTime = now
  return cachedData
}
...
function getMockTaoguba(): HotStock[] {
  return [
