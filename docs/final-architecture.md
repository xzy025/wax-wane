...
> ⚠️ **历史文档(2026-06-01 口径,部分内容已过时)**:NestJS/LangChain 迁移已于 2026-06-07 放弃(apps/server 已删除,现为单一 Express :3002);Redis/Milvus/sql.js/vectra 均已从代码与依赖中移除(2026-07-06);阅读时以 README 与源码为准。
...
| 后端 | Express (NestJS 迁移已放弃) |
| 数据库 | PostgreSQL + pgvector |
| 缓存 | Redis |
| AI 框架 | 自研 agent loop(LangChain/DeepAgents 方案已放弃) |
...
> ⚠️ 上图为规划口径,与现实不符。实际拓扑(2026-07-06):**单一 Express :3002 + Vite dev :3000 + PostgreSQL :5432**;NestJS 已放弃、Redis 已移除。
...
# Nest Server — ⛔ 已删除(NestJS 迁移已于 2026-06-07 放弃,apps/server 目录不存在)
